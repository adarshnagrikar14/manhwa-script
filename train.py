...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lora_num", type=int, default=1,
                        help="number of the lora.")
    parser.add_argument("--cond_size", type=int, default=512,
                        help="size of the condition data.")
    parser.add_argument("--noise_size", type=int, default=1280,
                        help="max side of the training data.")
    parser.add_argument("--test_h", type=int, default=1024,
                        help="max side of the training data.")
    parser.add_argument("--test_w", type=int, default=1024,
                        help="max side of the training data.")
    parser.add_argument("--mode", type=str, default=None,
                        help="The mode of the controller. Choose between ['depth', 'pose', 'canny'].")
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default="",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "/tiamat-NAS/zhangyuxuan/datasets/benchmark_dataset/decoritems_woodenpot/0.png"],
...
def main(args):
        ...
        for name, attn_processor in transformer.attn_processors.items():
            ...
            if name.startswith("transformer_blocks") and layer_index in double_blocks_idx:
                ...
                for n in range(number):
                    ...
                    lora_attn_procs[name].q_loras[n].up.weight.data = lora_state_dicts.get(
                    ...
                    lora_attn_procs[name].k_loras[n].up.weight.data = lora_state_dicts.get(
                    ...
                    lora_attn_procs[name].v_loras[n].up.weight.data = lora_state_dicts.get(
    ...
    print(sum([p.numel() for p in transformer.parameters()
          if p.requires_grad]) / 1000000, 'parameters')
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                prompt_embeds, pooled_prompt_embeds, text_ids = encode_token_ids(
                    text_encoders, tokens, accelerator)
                prompt_embeds = prompt_embeds.to(
                    dtype=vae.dtype, device=accelerator.device)
                ...
                if args.subject_column is not None:
                    subject_pixel_values = batch["subject_pixel_values"].to(
                        dtype=vae.dtype)
                    subject_input = vae.encode(
                        subject_pixel_values).latent_dist.sample()
                    subject_input = (
                        subject_input - vae_config_shift_factor) * vae_config_scaling_factor
                    ...
                    latent_subject_ids = prepare_latent_subject_ids(
                        height_cond, width_cond, accelerator.device, weight_dtype)
                    latent_subject_ids[:, 1] += offset
                    sub_latent_image_ids = torch.concat(
                    ...
                    packed_cond_model_input_to_concat.append(
                        packed_subject_model_input)

                if args.spatial_column is not None:
                    cond_pixel_values = batch["cond_pixel_values"].to(
                        dtype=vae.dtype)
                    cond_input = vae.encode(
                        cond_pixel_values).latent_dist.sample()
                    cond_input = (
                        cond_input - vae_config_shift_factor) * vae_config_scaling_factor
                ...
                latent_image_ids = torch.concat(
                    latent_image_ids_to_concat, dim=-2)
                cond_packed_noisy_model_input = torch.concat(
                    packed_cond_model_input_to_concat, dim=-2)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        lora_state_dict = {
                            k: unwrapped_model_state[k] for k in unwrapped_model_state.keys() if '_lora' in k}
                        save_file(
                            lora_state_dict,
            ...
            if accelerator.is_main_process:
                if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                    ...
                    if len(args.subject_test_images) != 0 and args.subject_test_images != ['None']:
                        ...
                        subject_ls = [Image.open(image_path).convert(
                            "RGB") for image_path in subject_paths]
                    else:
                        subject_ls = []
                    if len(args.spatial_test_images) != 0 and args.spatial_test_images != ['None']:
                        spatial_paths = args.spatial_test_images
                        spatial_ls = [Image.open(image_path).convert(
                            "RGB") for image_path in spatial_paths]
                    else:
                        spatial_ls = []
                    ########
                    ...
                    save_folder = os.path.join(
                        save_path, f"checkpoint-{global_step}")
